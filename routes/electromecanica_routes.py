...
def historial_exportaciones():
    ...
    files = list(db.fs.files.find(
        {"filename": {"$regex": "^Datos_Electromecanica_"}},
...
# 📌 Ruta para eliminar archivos de GridFS en ELECTROMECANICA
@electromecanica_bp.route('/historial/delete/<filename>', methods=['DELETE'])
@login_required('Electromecanica')
...
@electromecanica_bp.route("/exportar-manual", methods=["POST"])
@login_required("Electromecanica")
def exportar_manual():
    export_data_auto()
...
def export_selected():
    ...
    for i, profesor in enumerate(selected_profesores):
        ...
        header_image_copy = Image("static/electromecanica/src/Encabezado1.PNG")
        footer_image_copy = Image("static/electromecanica/src/PieDePagina1.PNG")
