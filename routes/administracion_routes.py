...
def historial_exportaciones():
    ...
    files = list(db.fs.files.find(
        {"filename": {"$regex": "^Datos_Administracion_"}},
...
# 📌 Ruta para eliminar archivos de GridFS en ADMINISTRACION
@administracion_bp.route('/historial/delete/<filename>', methods=['DELETE'])
@login_required('Administracion')
...
@administracion_bp.route("/exportar-manual", methods=["POST"])
@login_required("Administracion")
def exportar_manual():
    export_data_auto()
...
def export_selected():
    ...
    for i, profesor in enumerate(selected_profesores):
        ...
        header_image_copy = Image("static/administracion/src/Encabezado1.PNG")
        footer_image_copy = Image("static/administracion/src/PieDePagina1.PNG")
