...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Autocompletado para asignaturas normales (base de datos de electrónica)
    for (let i = 1; i <= 8; i++) {
        setupAsignatura(`asignatura${i}`, `horas${i}`, `asignatura-list${i}`, '/electronica/electronica_asignaturas/json');
    }

    // Autocompletado para asignaturas especiales (base de datos de electrónica)
    for (let i = 1; i <= 8; i++) {
...
document.addEventListener("DOMContentLoaded", function() {
    // LISTA DE GRUPOS PARA ELECTRÓNICA (Comienzan con 3)
