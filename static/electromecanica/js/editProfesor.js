document.getElementById('edit-profesor-form').addEventListener('submit', function(e) {
    ...
    // Enviar la solicitud PUT a la API de Electromecánica
    fetch(`/electromecanica/electromecanica_profesores/${profesorId}`, {  // 🔹 Ruta actualizada para Electromecánica
        method: 'PUT',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Autocompletado para asignaturas normales (base de datos de electromecánica)
    for (let i = 1; i <= 8; i++) {
        setupAsignatura(`asignatura${i}`, `horas${i}`, `asignatura-list${i}`, '/electromecanica/electromecanica_asignaturas/json');
    }

    // Autocompletado para asignaturas especiales (base de datos de electromecánica)
    for (let i = 1; i <= 8; i++) {
        setupAsignatura(`asignaturaE${i}`, `horasE${i}`, `asignaturaE-list${i}`, '/electromecanica/electromecanica_asignaturasE/json');
    }
});
...
document.addEventListener("DOMContentLoaded", function() {
    // LISTA DE GRUPOS PARA ELECTROMECÁNICA (Comienzan con 2)
    const gruposElectromecanica = ["", "NG", "2101", "2102", "2151", "2152", "2201", "2202", "2251", "2252",
        "2301", "2302", "2351", "2352", "2401", "2402", "2451", "2452", "2501",
        "2502", "2551", "2552", "2601", "2602", "2651", "2652", "2701", "2702",
        "2751", "2752", "2801", "2802", "2851", "2852", "2901", "2902"
    ];
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    carreraSelects.forEach(select => populateCareerSelectOptions(select, carrerasElectromecanica));
});

/// **VALIDACIÓN DE HORAS ASIGNADAS - ELECTROMECÁNICA**
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    grupoSelects.forEach((select, index) => {
        ...
        // Mostrar el select de días si el grupo termina en "21" (correspondiente a Electromecánica)
        function actualizarSelector() {
            const grupoValue = select.value.trim();
            if (grupoValue.endsWith("21")) { 
